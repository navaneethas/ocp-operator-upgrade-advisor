#!/usr/bin/env python3
"""
OpenShift Operator Upgrade Advisor - Standalone CLI
...
def analyze_json_file(json_file, target_ocp):
    """Analyze JSON file and display results"""
    try:
        # Load JSON file
        ...
        print(f"{Colors.FAIL}Error: File '{json_file}' not found{Colors.ENDC}")
        return 1
...
def main():
    parser = argparse.ArgumentParser(
        description='OpenShift Operator Upgrade Advisor - Standalone CLI (No Dependencies)',
...
Examples:
  ...
  %(prog)s openshift-data.json
    ...
    parser.add_argument('json_file', help='Path to JSON file containing cluster data')
    ...
    return analyze_json_file(args.json_file, target_ocp)
